      ...
      <main id="home">
        ...
        <HomeHeroSlideshow>
          <div className="flex min-h-svh flex-col justify-end px-6 pb-16 pt-28 sm:px-10 sm:pb-20 md:pb-24 lg:px-16">
            ...
            <div className="mt-6 flex w-full max-w-md flex-row flex-wrap gap-2 sm:mt-10 sm:max-w-none sm:gap-3 md:flex-row">
              <Link
                href="/portfolio"
                className="font-display inline-flex min-h-9 flex-1 touch-manipulation items-center justify-center border border-white/40 bg-white/95 px-3 py-2 text-sm text-[#3d342c] transition hover:bg-white sm:min-h-12 sm:w-auto sm:flex-none sm:px-8 sm:py-3.5 sm:text-xl md:text-[1.35rem] md:py-4 md:px-9"
              >
                View work
              </Link>
              <Link
                href="/contact"
                className="font-display inline-flex min-h-9 flex-1 touch-manipulation items-center justify-center border border-white/35 bg-transparent px-3 py-2 text-sm text-white transition hover:bg-white/10 sm:min-h-12 sm:w-auto sm:flex-none sm:px-8 sm:py-3.5 sm:text-xl md:text-[1.35rem] md:py-4 md:px-9"
              >
                Inquire
              </Link>
            </div>
        ...
        >
          <div className="mx-auto max-w-[2000px]">
            <div className="grid grid-cols-3 gap-1.5 px-3 pt-3 md:gap-0 md:px-0 md:pt-0">
              <div className="relative h-[4.75rem] overflow-hidden rounded-md sm:h-28 md:aspect-[4/5] md:h-auto md:min-h-[min(85vh,900px)] md:rounded-none">
                <Image
        ...
        >
          <div className="mx-auto grid max-w-6xl grid-cols-[minmax(0,38%)_minmax(0,1fr)] items-start gap-3 sm:grid-cols-2 sm:items-center sm:gap-10 lg:gap-20">
            <div className="relative aspect-[3/4] w-full min-w-0 sm:max-w-lg lg:max-w-none">
              <Image
                src={SITE_IMAGES.photographer}
                alt="Taylor — photographer"
                fill
                className="object-cover object-[center_15%]"
                sizes="(max-width: 640px) 38vw, (max-width: 1024px) 50vw, 50vw"
              />
            </div>
        ...
        >
          <div className="mx-auto max-w-6xl">
            ...
            <div className="columns-2 gap-x-2 gap-y-1 sm:gap-x-8 sm:gap-y-2 md:columns-2 lg:columns-3 lg:gap-x-10">
              {PORTFOLIO_HOME_CARDS.map((card, i) => {
                ...
                return (
                  <Link
                    key={card.name}
                    href={card.href}
                    className={`group mb-4 block break-inside-avoid transition duration-300 hover:z-10 hover:scale-[1.02] hover:rotate-0 max-md:rotate-0 max-md:translate-y-0 sm:mb-10 ${s.rotate} ${s.push}`}
                  >
                    <div
                      className={`scrapbook-mat rounded-[2px] bg-[#faf8f4] p-2 dark:bg-[#2a2622] max-md:!p-1 max-md:!pb-3 ${s.lip}`}
                    >
                      <div
